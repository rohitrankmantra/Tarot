...
export default function CheckoutForm() {
  ...
  const handleSubmit = (e) => {
    ...
    // Later we’ll send this to backend (orders API)
  };
      ...
      <div className="space-y-5">
          ...
          <div key={name}>
            <input
              ...
              className={`w-full px-4 py-3 bg-background/30 text-accent rounded-2xl shadow-inner focus:outline-none focus:ring-2 ${
                errors[name] ? "focus:ring-red-500 border border-red-500" : "focus:ring-accent"
              }`}
            />
            {errors[name] && (
        ...
        <div>
          <textarea
            ...
            rows={2}
            className={`w-full px-4 py-3 bg-background/30 text-accent rounded-2xl shadow-inner focus:outline-none focus:ring-2 ${
              errors.address ? "focus:ring-red-500 border border-red-500" : "focus:ring-accent"
            }`}
          />
          {errors.address && (
